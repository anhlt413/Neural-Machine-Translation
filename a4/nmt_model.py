...
class NMT(nn.Module):
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        _, (dec_hidden, dec_cell) = self.decoder(Ybar_t.unsqueeze(0), (hidden,cell))
        e_t = torch.bmm(enc_hiddens_proj,dec_hidden.squeeze(0).unsqueeze(-1)).squeeze(-1)
        dec_state = (dec_hidden.squeeze(0),dec_cell.squeeze(0))
        ###       Hints:
        ...
        u_t = torch.cat([dec_hidden.squeeze(0), a_t], dim = -1)
